...
- ``famous``: values users actually type: 0.1, 0.2, pi, e, 1/3.
...
FAMOUS = [
    0.1, 0.2, 0.1 + 0.2, 1/3, 1/7,
    math.pi, math.e, math.sqrt(2), math.log(2), 1.0 / math.pi,
    0.0001, 1e-10, 1e-100, 1e10, 1e100,
    1.7976931348623157e308, 2.2250738585072014e-308, 5e-324,
    float(2**53 - 1), float(2**53), float(2**53 + 2),
]


def famous(n: int = 10_000) -> list[float]:
    return (FAMOUS * ((n // len(FAMOUS)) + 1))[:n]
