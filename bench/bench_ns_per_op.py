...
single repr() or f-string interpolation, in nanoseconds. Runs many
repetitions, picks the median of the fastest third of runs to reduce
noise from jitter.
...
def main() -> None:
    ...
    for variant, setup in [("stock", floatium.uninstall),
        ...
        for corpus_name, values in data.items():
            for op_name, kernel in OPERATIONS.items():
                # Warm up once so code/data caches are hot for the measured
                # run; don't count the warmup toward the sample.
                kernel(values)
