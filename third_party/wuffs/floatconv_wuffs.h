/*
 * floatconv_wuffs.h -- Wuffs float-parsing code, vendored into CPython.
 *
 * Upstream:  https://github.com/google/wuffs
 * Commit:    988af1c03c6a23f3a90bd5f2c8529fd3cfa8daeb
 * License:   Apache License 2.0  OR  MIT License  (dual-licensed)
 *            See LICENSE-Apache2 and LICENSE-MIT in this directory.
 *
 * Extract of Wuffs single-file release + floatconv-submodule-code.c,
 * containing only the string-to-double parsing code:
 *   High Precision Decimal (HPD), Eisel-Lemire, parse_number_f64.
 *
 * Local modifications
 * -------------------
 * 1. Removed all code outside string-to-double parsing.
 * 2. Added self-contained type definitions and utility inlines.
 * 3. Replaced WUFFS_BASE__MAYBE_STATIC with static.
 * 4. Replaced extern const char declarations with static definitions.
 * 5. Added _MSC_VER/_M_X64 branch to multiply_u64 and clz_u64.
 * 6. No algorithmic changes.
 *
 * Public entry point:
 *   wuffs_base__result_f64 wuffs_base__parse_number_f64(
 *       wuffs_base__slice_u8 s, uint32_t options);
 * options==0 for standard decimal. ret.status.repr==NULL on success.
 */

#ifndef Py_FLOATCONV_WUFFS_H
#define Py_FLOATCONV_WUFFS_H

#include <stdint.h>
#include <stdbool.h>
#include <string.h>   /* memcpy */

/* ---- Minimal Wuffs type definitions ---- */

typedef struct { const char *repr; } wuffs_base__status;

static inline wuffs_base__status
wuffs_base__make_status(const char *repr) {
    wuffs_base__status z; z.repr = repr; return z;
}
...
#define WUFFS_BASE__RESULT(T) struct { wuffs_base__status status; T value; }
...
wuffs_base__make_slice_u8(uint8_t *ptr, size_t len) {
    wuffs_base__slice_u8 ret; ret.ptr = ptr; ret.len = len; return ret;
}

#define WUFFS_BASE__PARSE_NUMBER_FXX__DECIMAL_SEPARATOR_IS_A_COMMA 0x00100
#define WUFFS_BASE__PARSE_NUMBER_FXX__REJECT_INF_AND_NAN           0x00200
#define WUFFS_BASE__PARSE_NUMBER_XXX__ALLOW_MULTIPLE_LEADING_ZEROES 0x00800
#define WUFFS_BASE__PARSE_NUMBER_XXX__ALLOW_UNDERSCORES             0x01000
#define WUFFS_BASE__RENDER_NUMBER_FXX__JUST_ENOUGH_PRECISION        0x00004
...
wuffs_base__ieee_754_bit_representation__from_u64_to_f64(uint64_t u) {
    double f = 0;
    if (sizeof(uint64_t) == sizeof(double)) { memcpy(&f, &u, sizeof(uint64_t)); }
    return f;
}

typedef struct { uint64_t lo; uint64_t hi; } wuffs_base__multiply_u64__output;
...
wuffs_base__multiply_u64(uint64_t x, uint64_t y) {
    ...
    o.lo = (uint64_t)z; o.hi = (uint64_t)(z >> 64); return o;
#elif defined(_MSC_VER) && (defined(_M_X64) || defined(_M_ARM64))
#include <intrin.h>
    wuffs_base__multiply_u64__output o;
    o.lo = _umul128(x, y, &o.hi); return o;
#else
    uint64_t x0=x&0xFFFFFFFF, x1=x>>32, y0=y&0xFFFFFFFF, y1=y>>32;
    uint64_t w0=x0*y0, t=(x1*y0)+(w0>>32), w1=t&0xFFFFFFFF, w2=t>>32;
    w1 += x0*y1;
    wuffs_base__multiply_u64__output o;
    o.lo = x*y; o.hi = (x1*y1)+w2+(w1>>32); return o;
#endif
}
...
wuffs_base__count_leading_zeroes_u64(uint64_t u) {
    unsigned long idx;
    return _BitScanReverse64(&idx, u) ? (uint32_t)(63 - idx) : 64;
}
#else
static inline uint32_t
wuffs_base__count_leading_zeroes_u64(uint64_t u) {
    if (!u) return 64;
    uint32_t n = 0;
    if (!(u>>32)){n|=32;u<<=32;} if (!(u>>48)){n|=16;u<<=16;}
    if (!(u>>56)){n|= 8;u<<= 8;} if (!(u>>60)){n|= 4;u<<= 4;}
    if (!(u>>62)){n|= 2;u<<= 2;} if (!(u>>63)){n|= 1;}
    return n;
}
#endif

/* ---- Lookup tables (from Wuffs, unmodified) ---- */
